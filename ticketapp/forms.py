...
class Doc_Update_Form(forms.ModelForm):
    ...
    code = forms.CharField(
        max_length=100, widget=forms.Textarea(attrs={'class': 'form-control'}))
    field_section = forms.ChoiceField(
        choices=FIELD_SECTIONS, widget=forms.Select(attrs={'class': 'form-control'}))
    
    class Meta:
        model = Ticket
        exclude = ('user', 'doc_id', 'created_date',
                   'created_by')
